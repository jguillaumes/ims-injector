...
func main() {
	...
	flag.BoolFunc("v", "Enable verbose logging (default: false)", func(s string) error { return nil })
	...
	irm_template.Irm_timer = uint8(*timeout)
	irm_template.Irm_clientid = *clientID
	...
	inc := make(chan string, 100)  // Channel for incoming messages
	outc := make(chan string, 100) // Channel for outgoing messages
	errc := make(chan error, 1)    // Channel for errors & control

	// Start the interaction goroutine
	...
	go func() {
		...
		for {
			...
			case resp := <-outc:
				if resp == "" {
					// If an empty response is received, exit silently (and end the goroutine)
					ctrl <- struct{}{}
					return
				}
				numOK++
				// Write the response to the output file
				_, err := outputFile.WriteString(resp + "\n")
				if err != nil {
			...
			case err := <-errc:
				if err != nil {
					...
					log.Debug("Interaction completed successfully")
				}
				// Exit the loop if an error is received or the interaction is complete
				ctrl <- struct{}{}
				return
			}
		}
